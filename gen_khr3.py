...
def arm(name,xyz,whd,mass,color,*options,rpy=(0,0,0)):
    ...
    if 'imu' in opts:
        keys['sensors'] = 'imu'
    keys['color'] = color
    return name,xyz,rpy,whd,mass,**keys
...
def arms(baselink,name,profile,RorL):
    ...
    for i,p in enumerate(profile):
        gs.link_box(arm(linknames[i+1],*p[:-3]))
        gs.joint_revolute(armname+'r'+str(i),*linknames[i:i+2],*p[-3:])
...
arms_profile = [((0.02,0,0),(0.03,0.045,0.02),0.02,'Black',(0.045,0.01,0.045),x,limits),
                ((0,0,-0.03),(0.02,0.03,0.08),0.1,'Black',['selfCollide'],(0.025,0,0),y,limits),
                ((0,0,-0.015),(0.03,0.04,0.02),0.05,'Black',(0,0,-0.07),z,limits),
                ((0,0,-0.05),(0.03,0.02,0.06),0.03,'White',['selfCollide'],(0,0.006,-0.015),x,limits)]
arms('chest','Rarm',arms_profile,-1)
arms('chest','Larm',arms_profile,1)

#legs
legs_profile = [((0,0,-0.015),(0.02,0.05,0.03),0.015,'White',(0.023,0.01,-0.02),z,(effort,-0.2*pi,0.2*pi,vel)),
                ((0.012,0,0),(0.04,0.03,0.02),0.065,'Black',(-0.006,0,-0.022),y,(effort,-0.2*pi,0.2*pi,vel)),
                ((0,0,-0.03),(0.04,0.02,0.08),0.1,'Black',['selfCollide'],(0.012,0,-0.04),x,limits),
                ((0,0,-0.04),(0.03,0.02,0.04),0.065,'Black',(0,0,-0.065),x,limits),
                ((0,0,-0.02),(0.04,0.03,0.06),0.065,'Black',(0,0,-0.065),x,limits),
                ((0.017,0,-0.01),(0.05,0.05,0.03),0.045,'White',['selfCollide'],(-0.012,0,-0.04),y,(effort,-0.4*pi,0.4*pi,vel))]
